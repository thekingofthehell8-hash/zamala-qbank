...
def reset_exam(selected_df: pd.DataFrame, mode_label: str, topic_filter_label: str, bank_label: str, timer_minutes: int = 0):
    ...
    st.session_state.timer_minutes = int(timer_minutes or 0)
    st.session_state.timer_enabled = st.session_state.timer_minutes > 0
...
def get_timer_state():
    if not st.session_state.get("timer_enabled") or not st.session_state.get("exam_start_iso"):
        return None
    try:
        start_dt = datetime.fromisoformat(st.session_state.exam_start_iso)
    except Exception:
        return None

    duration_seconds = int(st.session_state.get("timer_minutes", 0) * 60)
    if duration_seconds <= 0:
        return None

    end_dt = start_dt + timedelta(seconds=duration_seconds)
    remaining_seconds = int((end_dt - datetime.now()).total_seconds())
...
with st.sidebar:
    ...
    default_timer = 120 if mode_choice in {"Selected Bank", "Random Mock (100)"} else 25 if mode_choice == "Mini Test (20)" else 0
    timer_minutes = st.number_input("Timer (minutes, 0 = off)", min_value=0, max_value=600, value=default_timer, step=5)
    ...
    if st.button("Start / Restart", use_container_width=True):
        ...
        if filtered_df.empty:
            ...
            reset_exam(filtered_df, mode_choice, topic_filter_label, current_bank_label, timer_minutes=timer_minutes)
        elif mode_choice == "Random Mock (100)":
            n = min(100, len(filtered_df))
            reset_exam(filtered_df.sample(n=n).reset_index(drop=True), mode_choice, topic_filter_label, current_bank_label, timer_minutes=timer_minutes)
            ...
            if wrong_df.empty:
                ...
                reset_exam(wrong_df, mode_choice, topic_filter_label, current_bank_label, timer_minutes=timer_minutes)
        else:
    ...
    if current_df is not None and not current_df.empty and st.session_state.started:
        q_labels = [
            f"{'🚩 ' if st.session_state.get('flags', {}).get(i, False) else ''}{i + 1}. Q{int(qid)} — {bank}"
...
if timer_state and not st.session_state.review_mode and not st.session_state.submitted:
    if timer_state["expired"] and not st.session_state.timer_expired:
...
info_cols[0].metric("Answered", f"{answered_count()}/{len(current_df)}")
info_cols[1].metric("Remaining", f"{len(current_df) - answered_count()}")
info_cols[2].metric("Flagged", f"{count_flagged()}")
if timer_state and not st.session_state.review_mode:
    info_cols[3].metric("Time Left", format_seconds(timer_state["remaining_seconds"]))
else:
    info_cols[3].metric("Mode", st.session_state.mode)

flag_current = st.checkbox("🚩 Flag this question", value=st.session_state.get("flags", {}).get(idx, False), key=f"flag_{idx}")
st.session_state.flags[idx] = flag_current
...
with nav2:
    if st.button("Next", disabled=idx == len(current_df) - 1, use_container_width=True):
        ...
        st.rerun()
with nav3:
    flagged_indices = [i for i, v in st.session_state.get("flags", {}).items() if v]
    next_flagged = next((i for i in flagged_indices if i > idx), None)
    if next_flagged is None and flagged_indices:
        next_flagged = flagged_indices[0]
    if st.button("Next Flagged", disabled=not flagged_indices, use_container_width=True):
        st.session_state.current_idx = next_flagged
        st.rerun()
with nav4:
...
if not st.session_state.review_mode:
    st.write(f"Answered: **{answered_count()}/{len(current_df)}** | Flagged: **{count_flagged()}**")
    flagged_qids = [str(int(current_df.iloc[i]["question_id"])) for i, v in st.session_state.get("flags", {}).items() if v and i < len(current_df)]
    if flagged_qids:
        st.caption("Flagged question IDs: " + ", ".join(flagged_qids))
else:
...
if st.session_state.get("timer_expired") and st.session_state.submitted and not st.session_state.review_mode:
    st.warning("Time is up. The exam was submitted automatically.")
