...
def discover_exam_files() -> list[dict]:
    ...
    def add_record(path: Path, label: str, sort_key: int):
    ...
    for base in DATA_DIR_CANDIDATES:
        ...
        for path in sorted(base.glob("module_*_exam_*.csv")):
            ...
            if m:
                ...
                label = path.stem
                sort_key = 9999
            add_record(path, label, sort_key)

        predicted_patterns = [
            "questions_predicted_exam_*.csv",
            "predicted_exam*.csv",
        ]
        for pattern in predicted_patterns:
            for path in sorted(base.glob(pattern)):
                ...
                if m:
                    question_count = int(m.group(1))
                label = f"Predicted Exam ({question_count})" if question_count else path.stem
                add_record(path, label, 9998)

        fallback = base / "questions.csv"
        if fallback.exists():
            add_record(fallback, "questions.csv", 10000)
...
if all_df.empty:
    st.error("No question CSV files were found. Put the six module CSV files and any predicted exam CSV next to the app file or inside a data folder.")
    st.stop()
