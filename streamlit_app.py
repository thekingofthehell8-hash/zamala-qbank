...
def discover_exam_files() -> list[dict]:
    ...
    for base in DATA_DIR_CANDIDATES:
        ...
        for path in sorted(base.glob("questions_predicted_exam_*.csv")):
            resolved = path.resolve()
            if resolved in seen or not csv_has_any_content(path):
                continue
            seen.add(resolved)
            m = re.search(r"questions_predicted_exam_(\d+)", path.stem, re.IGNORECASE)
            count_hint = m.group(1) if m else ""
            label = f"Predicted Exam ({count_hint})" if count_hint else "Predicted Exam"
            records.append({
                "bank_id": path.stem,
                "bank_label": label,
                "path": str(path),
...
def parse_iso(x: str):
    try:
        return datetime.fromisoformat(normalize_text(x))
    except Exception:
        return None


def get_elapsed_seconds() -> int:
    start = parse_iso(st.session_state.get("exam_start_iso", ""))
    end = parse_iso(st.session_state.get("exam_end_iso", ""))
    if not start:
        return 0
    if end:
...
def render_elapsed_timer():
    start_iso = normalize_text(st.session_state.get("exam_start_iso", ""))
    if not start_iso or st.session_state.review_mode:
        return

    frozen = normalize_text(st.session_state.get("exam_end_iso", ""))
    if frozen:
        st.info(f"⏱ Total time: {format_seconds(get_elapsed_seconds())}")
        return

    components.html(
        f"""
        <div id="elapsed-box" style="padding:10px 12px;border:1px solid #2b2b2b;border-radius:10px;background:#0e1117;color:#fafafa;font-family:Arial,sans-serif;font-weight:600;">
          ⏱ Exam time: <span id="elapsed-timer">00:00</span>
        </div>
        <script>
        ...
        function updateElapsed() {{
          ...
          const s = diff % 60;
          const out = h > 0 ? `${{pad(h)}}:${{pad(m)}}:${{pad(s)}}` : `${{pad(m)}}:${{pad(s)}}`;
          const node = document.getElementById("elapsed-timer");
          if (node) node.innerText = out;
        }}
        updateElapsed();
...
def format_answer_display(answer_value, row) -> str:
    answer = normalize_answer(answer_value)
    if not answer:
        return "No answer"
    ...
    for letter in letters:
        text = normalize_text(options.get(letter, ""))
        if text:
...
def reset_exam(selected_df: pd.DataFrame, mode_label: str, topic_filter_label: str, bank_label: str):
    ...
    st.session_state.exam_start_iso = datetime.now().isoformat(timespec="seconds")
    st.session_state.exam_end_iso = ""
...
def reset_review(selected_df: pd.DataFrame, topic_filter_label: str, bank_label: str):
    ...
    st.session_state.bank_label = bank_label
    st.session_state.exam_start_iso = ""
    st.session_state.exam_end_iso = ""
...
def flagged_count() -> int:
    return sum(1 for v in st.session_state.flags.values() if bool(v))


def next_flagged_index(current_idx: int):
    flagged = sorted(i for i, v in st.session_state.flags.items() if v)
    if not flagged:
        return None
    for i in flagged:
        if i > current_idx:
...
def render_dashboard(summary_df: pd.DataFrame, answers_df: pd.DataFrame, bank_df: pd.DataFrame):
    ...
    if "elapsed_seconds" in recent.columns:
        recent["elapsed"] = recent["elapsed_seconds"].apply(lambda x: format_seconds(pd.to_numeric(x, errors="coerce") if str(x).strip() else 0))
    with st.expander("Recent Attempts", expanded=False):
        cols = [c for c in ["timestamp", "bank_label", "mode", "topic_filter", "question_count", "score", "percentage", "elapsed"] if c in recent.columns]
...
if all_df.empty:
    st.error("No question CSV files were found. Put the six module CSV files and predicted CSV next to main.py or inside a data folder.")
    st.stop()
...
with st.sidebar:
    ...
    st.write(f"Flagged: **{flagged_count()}**")
...
if not st.session_state.review_mode:
    ...
    c1.metric("Answered", f"{answered_count()}/{len(current_df)}")
    c2.metric("Remaining", f"{len(current_df) - answered_count()}")
    c3.metric("Flagged", f"{flagged_count()}")
    c4.metric("Elapsed", format_seconds(get_elapsed_seconds()))
    render_elapsed_timer()
...
if st.session_state.review_mode:
    ...
    st.success(f"Correct answer: {format_answer_display(row.get('correct_answer', ''), row)}")
    if normalize_text(row.get("explanation", "")):
        st.info(row["explanation"])
else:
    st.checkbox("🚩 Flag this question", key=f"flag_{idx}", value=bool(st.session_state.flags.get(idx, False)))
    st.session_state.flags[idx] = bool(st.session_state.get(f"flag_{idx}", False))
...
with nav3:
    if not st.session_state.review_mode:
        nxt = next_flagged_index(idx)
        if st.button("Next Flagged", disabled=nxt is None, use_container_width=True):
    ...
    if not st.session_state.review_mode:
        if st.button("Submit Exam", use_container_width=True):
            ...
            if unanswered > 0 and not st.session_state.submit_confirm:
                ...
                st.session_state.submit_confirm = False
                st.session_state.exam_end_iso = datetime.now().isoformat(timespec="seconds")
            st.rerun()

if not st.session_state.review_mode:
    st.write(f"Answered: **{answered_count()}/{len(current_df)}** | Flagged: **{flagged_count()}** | Elapsed: **{format_seconds(get_elapsed_seconds())}**")
else:
...
if st.session_state.submitted and not st.session_state.review_mode:
    ...
    m4.metric("Time Taken", format_seconds(elapsed_seconds))
    ...
    if wrong_df.empty:
        ...
        for _, item in wrong_df.iterrows():
            original_row = current_df[current_df["question_uid"].astype(str) == str(item["question_uid"])]
            original_row = original_row.iloc[0].to_dict() if not original_row.empty else item.to_dict()
            st.markdown(f"### {item['bank_label']} — Q{int(item['question_id'])}")
            st.write(item["stem"])
            st.write(f"**Your answer:** {format_answer_display(item['user_answer'], original_row)}")
            st.write(f"**Correct answer:** {format_answer_display(item['correct_answer'], original_row)}")
